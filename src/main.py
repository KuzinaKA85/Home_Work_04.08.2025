...
def get_transaction_data(file_path: Path, file_type: str) -> list[dict[Any, Any]]:
    """
...
def is_reverse() -> bool:
    ...
    """
    messages = "Отсортировать по возрастанию или по убыванию (В/У)?"
    print(messages)
    while True:
        user_message = input()
        if "У" in user_message.upper():
            return True
        if "В" in user_message.upper():
            return False
        else:
            print(f'Ваш ответ "{user_message}" некоректен')
            print("Введите по по возрастанию или по убыванию (В/У)")
